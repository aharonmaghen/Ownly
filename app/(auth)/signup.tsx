...
export default function SignupScreen() {
  ...
  const handleSignUp = () => {
    clearError();
    signUp(email.trim(), password, mode === 'join' ? inviteCode : undefined);
  };
    ...
    <SafeAreaView className="flex-1 bg-surface">
      ...
      >
        ...
        <View className="bg-white rounded-3xl p-6" style={{ shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 12, elevation: 3 }}>
          <Text className="text-xl font-bold text-slate-800 mb-4">Create account</Text>
          ...
          {mode === 'join' && (
            <>
              <Text className="text-sm font-medium text-slate-600 mb-1">Invite code</Text>
              <TextInput
                value={inviteCode}
